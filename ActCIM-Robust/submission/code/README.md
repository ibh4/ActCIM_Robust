...
## 最佳模型权重 | Best Checkpoints

最佳模型权重文件因体积过大（每个约 85MB），未包含在提交包中。请按下方复现指令重新训练生成。

| Checkpoint | 路径 | 大小 |
|-----------|------|------|
| Clean baseline (seed 42) | `results/baseline/seed_42/best.pt` | ~85MB |
| Fixed-NAT (seed 42) | `results/fixed_nat/fixed_nat/seed_42/best.pt` | ~85MB |

> 如需评审权重文件，可提供单独下载链接（如百度网盘/Google Drive）。
